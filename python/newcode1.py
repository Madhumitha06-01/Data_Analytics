...
highlights_pattern = r"Highlights:\s*:\s*([\s\S]*?)(?=\nFeatures|\n\+91)"
features_pattern = r"Features\n(.+?)(?=\n\+91|\n)"
mrp_pattern = r"MRP\s+(\d+/-(?=\n))"
material_pattern = r"Material\s*:\s*(.+?)(?=\n)"
dimensions_pattern = r"Width\s*:\s*(\d+\s*CM)\s*Length\s*:\s*(\d+\s*CM)\s*Height\s*:\s*(\d+\s*CM)"
...
for page_num in range(1,3):
    ...
    # Extract dimensions using the dimensions pattern
    dimensions = "N/A"  # Default value if no dimensions are found
    dimension_match = re.search(dimensions_pattern, text)
    if dimension_match:
        dimensions = " x ".join(dimension_match.groups())
